    ...
    func content() -> some View {
        switch selectedKeyPath {
            case \.gestationalWeek:
                GestationalWeekGrid() { week in healthInfo.gestationalWeek = week ?? .early }
            case \.height:
            ...
            case \.bloodPressure:
                BloodPressureGrid() { bloodPressure in healthInfo.bloodPressure = bloodPressure ?? .none }
            case \.diabetes:
                DiabetesGrid() { diabetes in healthInfo.diabetes = diabetes ?? .none }
            case \.restrictions:
                FoodRestrictionGrid() { allergies in healthInfo.restrictions = allergies }
            default: EmptyView()
        }
    }
}
